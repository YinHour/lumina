...
- **Checkpointing**: Each graph (chat, source_chat) uses a separate SQLite checkpoint file to avoid concurrent write conflicts. Module-level graphs use `MemorySaver`; API routers use per-request `AsyncSqliteSaver` pointing to dedicated files (`chat_checkpoints.sqlite`, `source_chat_checkpoints.sqlite`).
...
- **Checkpoint file locations**: `LANGGRAPH_CHAT_CHECKPOINT_FILE` (`chat_checkpoints.sqlite`) and `LANGGRAPH_SOURCE_CHAT_CHECKPOINT_FILE` (`source_chat_checkpoints.sqlite`); graphs never share a checkpoint file

## Key Dependencies

- `langgraph`: StateGraph, Send, END, START, MemorySaver and AsyncSqliteSaver checkpoint persistence

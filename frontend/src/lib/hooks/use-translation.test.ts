...
describe('useTranslation Hook', () => {
  ...
  beforeEach(() => {
    ...
    ;(useI18nTranslation as unknown as { mockReturnValue: (v: unknown) => void }).mockReturnValue({
      t: (key: string) => {
        if (key === 'common') return { appName: 'Lumina™ | Yinshi AI' }
        if (key === 'common.appName') return 'Lumina™ | Yinshi AI'
        return key
      },
      i18n: {
        language: 'en-US',
  ...
  it('should return initial translations via proxy', () => {
    ...
    expect(result.current.t.common.appName).toBe('Lumina™ | Yinshi AI')
  })
